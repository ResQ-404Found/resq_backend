    ...
    def list_contacts(self) -> List[Dict]:
        ...
        for r in rows:
            ...
            out.append({
                ...
                "relation": r.relation,
                "is_emergency": True,  # 레코드 존재 = True
                "target_username": getattr(tu, "username", None),
                "target_profile_imageURL": getattr(tu, "profile_imageURL", None),
            })
        return out
    def list_broadcasts(self, limit: int = 20) -> List[Dict]:
            bcs = self.s.exec(
                select(EmergencyBroadcast)
                .where(EmergencyBroadcast.user_id == self.user_id)
                .order_by(EmergencyBroadcast.id.desc())
                .limit(limit)
            ).all()
            ...
            for bc in bcs:
                recs = self.s.exec(
                    select(EmergencyBroadcastRecipient).where(
                        EmergencyBroadcastRecipient.broadcast_id == bc.id
                    )
                ).all()
                total = len(recs)
                ...
                out.append({
                    "id": bc.id,
                    "message": bc.message,
                    "include_location": bc.include_location,
                    "created_at": bc.created_at,
                    "total": total, "sent": sent, "failed": failed,
                })
            return out
    def get_broadcast_detail(self, broadcast_id: int) -> Dict:
        bc = self.s.get(EmergencyBroadcast, broadcast_id)
        if not bc or bc.user_id != self.user_id:
            raise ValueError("조회할 수 없는 브로드캐스트입니다.")
        ...
        for r in recs:
            contact = self.s.get(EmergencyContact, r.contact_id)
            target_username = None
            target_user_id = None
            if contact:
                target_user_id = contact.target_user_id
                u = self.s.get(User, contact.target_user_id)
                target_username = getattr(u, "username", None)

            recipients.append({
                "id": r.id,
                "contact_id": r.contact_id,
                "contact_target_user_id": target_user_id,
                "contact_target_username": target_username,
                "status": r.status,
                "error": r.error,
                "channel": r.channel,
            })
